...
#endif
#ifdef CONFIG_SCC_CLANG
	header->version = LLVM_INSTR_PROF_RAW_VERSION;
#else
	header->version = LLVM_INSTR_PROF_RAW_VERSION | LLVM_VARIANT_MASK_IR_PROF;
#endif
 ...
 * Note: caller *must* hold clang_instr_profile_lock.
 */
...
/* release() implementation for clang_instr_profile. Release resources allocated by open(). */
